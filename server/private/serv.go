...
import (
	...
	wiki_service "github.com/skygenesisenterprise/giteria/services/wiki"
)
...
func ServCommand(ctx *context.PrivateContext) {
	...
	if repoExist &&
		...
		if key.Type == asymkey_model.KeyTypeDeploy {
			...
			// The real permission check is done in HookPreReceive (server/private/hook_pre_receive.go).
